...
export default function Post({ post, id, updatePosts } : {post: Post, id: string, updatePosts: any}) {
  ...
  async function likePostFunc() {
    if (currentUser && session?.user?.id) {
      ...
      if (hasLiked) {
        ...
        // if there are no likes, switch back to false
  ...
  async function deletePostFunc() {
    if (window.confirm("Are you sure you want to delete this post?")) {
      ...
      if (updatePosts) updatePosts("delete", null, id); // optimistic update

      const res = await deletePost(id, session?.user?.accessToken);
      //rollback optimistic update if error
      if (!res) {
        toastError("Error deleting post", undefined);
        if (updatePosts) updatePosts("add", post, id);
      } else {
        toastSuccess("Post deleted", undefined);
        // navigate to the home page if the post is deleted from the post page
        if (!updatePosts && pathname.includes("posts")) router.back();
      }
    }
  }
      ...
      <div className="flex-1 md:ml-3 ml-2 mr-3 sm:mr-7 max-w-[calc(100vw-80px)]">
        ...
        >
          <p
            ...
            onClick={() => router.push(`/posts/${id}`)}
            className="text-gray-800 dark:text-darkText whitespace-break-spaces line-clamp-[10] overflow-hidden break-words max-w-[550px] text-[15px] sm:mr-2 sm:text-[16px] mb-2"
          >
            {post?.content}
          </p>
...

          {
            ...
            post?.url && !post?.url?.includes("youtube") && (
              <Image
                ...
                className="rounded-2xl max-h-80 w-auto sm:w-full object-cover"
                width={500}
                height={500}
                src={post?.url}
                alt="img"
        ...
        <div className="flex justify-between text-gray-500 ml-[-8px]">
          <div className="flex items-center select-none">
            ...
            />
            {
              ...
              <span data-testid="post-comment-count" className="text-sm">
