...
export default function Trending({ post }) {
  // open in same tab
  return (
    <a rel="noreferrer" href={`${"http://localhost:3000/posts/" + post?._id}`} target="_self" >
      <div className="flex items-center justify-between px-4 py-2 space-x-1 hover:bg-gray-200 dark:hover:bg-darkHover transition duration-500 ease-out">
        <div className="space-y-0.5">
          ...
          <h6 className="text-sm font-bold dark:text-darkText">{post?.content}</h6>
          {
          post?.tags?.length > 0 &&
          <p className="text-[13px] leading-4 font-medium text-gray-500 ">
                Trending in {
                  post?.tags.map((tag, index) => (
                    <span key={index} className="text-[13px] font-normal hover:underline text-primary whitespace-pre-wrap">
                      #{tag}{index !== post?.tags?.length - 1 && <span className="text-xs font-medium text-gray-500 ">, </span>}
                    </span>
                  ))
                }
          </p>
...

        {
          ...
          <Image
            ...
            unoptimized={true}
            loader={({ src }) => src }
            src={`https://img.youtube.com/vi/${post?.url?.split("v=")[1]}/0.jpg`}
            alt=""
            />
        }
